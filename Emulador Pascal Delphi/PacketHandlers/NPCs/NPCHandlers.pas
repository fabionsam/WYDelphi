...
begin
  ...
  begin
    ...
    begin
      npc.SendChat('São necessários 30 milhões de gold para compor o Sephirot.');
      exit;
    end;
...
begin
  ...
  begin
    npc.SendChat('Level inválido para realizar a quest.');
    exit;
  end;
  ...
  begin
    ...
    begin
      npc.SendChat('Onde está o item ' + ItemList[quest.ItemId].Name + '?');
      exit;
    end;
  end;
  ...
  begin
    npc.SendChat('Já tem gente de mais lá. É melhor você esperar.');
    exit;
  end;
...
begin
  if(player.Character.CapeInfo <> 7) OR (player.Character.CapeInfo <> 8) then
	begin
		npc.SendChat('Você não pertence a nenhum Reino.');
		exit;
	end;
	...
	begin
		npc.SendChat('Você precisa de ' + IntToStr(fame) + ' pontos de Fame.');
		exit;
	end;
  {
	INT16 Cape := 0;
...
begin
  ...
  if(uni = -1) then
  begin
    npc.SendChat('Traga a alma de unicórnio');
    exit;
  end;
  ...
  if(fenix = -1) then
  begin
    npc.SendChat('Traga a alma de fênix');
    exit;
  end;
  ...
  begin
    npc.SendChat('Você também precisa trazer 30 safiras');
    exit;
  end;
...

  ...
  else npc.SendChat('Você precisa alinhar as Almas.');
end;
...
begin
  ...
  begin
    npc.SendChat('Nível permitido: 201 ao 250.');
    exit;
  end;
...
begin
	...
	begin
		npc.SendChat('Somente do nível 300 acima.');
		exit;
	end
	...
	begin
		npc.SendChat('Onde está a Imortalidade ?');
		exit;
	end
	else if (player.Character.Equip[11].Index < 1760)
    OR (player.Character.Equip[11].Index > 1763) then
	begin
		npc.SendChat('Traga-me um sephirot para a composição.');
		exit;
	end;
  ...
  begin
	...
	if(freeSlot = -1) then
	begin
		npc.SendChat('Você não tem espaço suficiente para um novo personagem.');
		exit;
	end;
...
begin
	...
	begin
		npc.SendChat('Necessário possuir 200 de Fame para prosseguir.');
		exit;
	end;
...
begin
  ...
  begin
    ...
    player.SendEquipItems();
		...
		npc.SendChat('Parabéns, adquiriu o Manto do Aprendiz.');
	end;
end;
...
begin
  ...
  begin
    npc.SendChat('Somente mortais entre os níveis 5 e 255.');
    exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        npc.SendChat('É necessário que a refinação do item seja +3');
        exit;
      end;
  ...
  npc.SendChat('Acho que esse prêmio irá agrada-lo');
end;
...
begin
	...
	begin
		npc.SendChat('Você precisa ser mortal e ter um nível inferior a 35.');
		exit;
	end;
