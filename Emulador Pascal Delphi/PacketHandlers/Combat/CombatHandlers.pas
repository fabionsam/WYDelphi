...
Uses PlayerData, BaseMob, MiscData, ItemFunctions, Log,
  ...
  type TCombatHandlers = class(TObject)
    ...
    public
      ...
      class function GetSkillDamage(attacker : TBaseMob; SkillId, weather, weapondamage: integer): integer; static;

      class function HandleAttack(attacker : TBaseMob; targetId : WORD; skillId : Integer; cooldown: Boolean = true) : boolean; overload; static; inline;
      class function HandleSingleTarget(attacker: TBaseMob; var buffer : array of Byte) : boolean; static;
      class function HandleAoE(attacker: TBaseMob; var buffer : array of Byte) : boolean; static;

    private
      class function CheckSkill(const mob: TBaseMob; skillId : Int16): boolean; static;
      class function DiscountMP(attacker: TBaseMob; skillId : Int16) : boolean; static;
  end;
...
class function TCombatHandlers.HandleAoE(attacker: TBaseMob; var buffer: array of Byte): boolean;
...
class function TCombatHandlers.HandleSingleTarget(attacker: TBaseMob; var buffer: array of byte) : boolean;
...
class function TCombatHandlers.HandleAttack(attacker: TBaseMob; targetId: WORD; skillId: Integer; cooldown: Boolean): boolean;
var target : TBaseMob; damage : integer;
begin
  result := false;

  if not(Assigned(attacker)) or not(TBaseMob.GetMob(targetId, target)) then exit;

  if not(attacker.CheckCooldown(skillId)) then
    exit;

  if not CheckSkill(attacker, skillId) then exit;  //151 é ataque a distancia
  if not DiscountMP(attacker, skillId) then exit;

  attacker.AddToEnemyList(target);
  target.AddToEnemyList(attacker);
  result := true;

  if (skillId = -1) or (skillId in [151, 153]) then
  begin
...
begin
  ...
  begin
    if(mob.IsPlayer) then
      TPlayer.Players[mob.ClientId].SendClientMessage('Você não possui essa Skill');
    exit;
  end;
  result := true;
end;

class function TCombatHandlers.DiscountMP(attacker: TBaseMob; skillId: Int16): boolean;
var mp: Integer;
begin
...
class function TCombatHandlers.GetSkillDamage(attacker: TBaseMob; SkillId, weather, weapondamage: integer): integer;
var SkillInstanceType, SkillMaster, Level, MobAprend, SkillInstanceValue,
SkillAffectValue, calcSkillDmg, calcSkillNum: integer;
begin
	SkillInstanceType := SkillsData[SkillId].InstanceType;
